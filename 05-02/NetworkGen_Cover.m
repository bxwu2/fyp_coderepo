...
% numHopsUnique = unique(numHops);       % temp vector of vals
% numHopsSort = sort(numHops);          % sorted input aligns with temp (lowest to highest)
% numHopsArr = zeros(size(numHopsUnique)); % vector for freqs
% % frequency for each value
% for histo = 1:length(numHopsUnique)
%     numHopsArr(histo) = sum(numHops == numHopsUnique(histo));
% end
% 
% figure(2)
