...
figure(2)
subplot(2,2,1)
plot(0:tHop,arrRecur(1:tHop+1,nodeDst),'*')
...
% figure(3)
subplot(2,2,2)
plot(0:tHop, arrRecurCDF(1:length(0:tHop),nodeDst),'o')
...
% for tau = 1:t
%     commuteConvPDF(tau+1) = commuteConvPDF(tau+1)+commuteConvPDF(tau);    
% end
commuteConvPDF = zeros(1,t+1);
for iterTau = 1:t
    commuteConv =zeros(1,iterTau+1);
    for tau = 1:iterTau
        commuteConv(tau+1) = arrRecur(tau+1,nodeDst)*arrRecurReturn(t-tau+1,nodeSrc);
    end
%     commuteConvPDF = commuteConv;
    addTau = 0;
    for tau = 1:iterTau
        addTau = addTau + commuteConv(tau) 
%         commuteConvPDF(tau+1) = commuteConvPDF(tau+1)+commuteConvPDF(tau);    
    end
    commuteConvPDF(iterTau) = addTau;
end
...
commuteConvCDF = commuteConvPDF;
for commuteCDFIndex = 2:length(commuteConvPDF)
    commuteConvCDF(commuteCDFIndex) = commuteConvPDF(commuteCDFIndex) + commuteConvPDF(commuteCDFIndex-1);
end
figure(5)
plot(1:length(commuteConvPDF),commuteConvCDF,'*')
title('CDF of Commute_i_j')
ylabel('Probability (%)')
xlabel('t (hops)')
%     arrRecurCDF(cdfIndex,nodeDst) = arrRecurCDF(cdfIndex,nodeDst)+arrRecurCDF(cdfIndex-1,nodeDst);    
% 
