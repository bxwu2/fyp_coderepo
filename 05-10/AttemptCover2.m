...
% for coverTermN = 1:n
%     transMatrixN = transMatrix
%     coverTermNSum = zeros(1:n);
%     for coverAbs = 1:coverTermN
%         transMatrixN(nodeSrc,coverAbs) = 1;
%     end
%     transMatrixNT = transMatrixN^t;
%     for coverAbs = 1:coverTermN
%         coverTermNSum = coverTermNSum + transMatrixNT(nodeSrc,coverAbs);
%     end
%     
% end

% new thinking???
% for 2nd term it could be referring to
% for i = 1, do j = 2 to n of Exi U Exj
% but do we do the neighbours????? probably? <-- ignore this for now
...
%         continue
%     end
%     transMatrixAbs(nodeSrc, cover_i) = 1;
%     for cover_j = cover_i+1:n
%         if (cover_j == nodeSrc)
%             continue
%         end
%         transMatrixAbs(nodeSrc, cover_j) = 1;
%         transMatrixN = transMatrixAbs^t;
%         coverTermNSum(cover_i) = coverTermNSum(cover_i) + transMatrix(nodeSrc,cover_i)+ ...
%             transMatrix(nodeSrc,cover_j);
%         
%     end
%     
% end
...
%     termNsum = 0;
    ...
    % %     termNsum = 0;
    %     for unionNumber = 1:termN
    %         termNsum = termNsum + (transMatrixT(nodeSrc,unionNumber);
    %     end
        termNsum(termN) = sum(transMatrixT(nodeSrc,1:unionNumber));
        arrUnion(termN) = termNsum(termN);
    end


        coverCDF(coverIndex) = term1(coverIndex);
        for coverSum = 1: n-1
            coverCDF(coverIndex) = coverCDF(coverIndex) +(-1)^coverSum *arrUnion(coverSum+1);
        end
end
