...
%            transMatrixT(nodeSrc,nodeSrc) = 0; % to not include z (doesnt
%            matter anymore? since we only extract them specifically in the
%            next line)
           
           combProbs = transMatrixT(nodeSrc,combArr(combIndex,:)); % asking supervisor
           combSum = combSum + sum(combProbs);
        end
        termNsum(union) = combSum;
%%         transMatrixAbs = transMatrix;
%         combArrEnd = 1:n;
%         transMatrixAbs(combArrEnd,:)= 0;
%         absIdx = sub2ind(size(transMatrix),combArrEnd, combArrEnd);
%         transMatrixAbs(absIdx) = 1;
%         
%         transMatrixT = transMatrixAbs^coverIndex;     
%         combProbs = transMatrixT(nodeSrc,combArr(combIndex,:));
%         combSum = combSum + sum(combProbs);
        
%         totalSum = totalSum + combSum;
...
%%
        ...
        coverCDF(coverIndex) = coverCDF(coverIndex) + (-1)^(n-1)*1;
end
