...
%% Term n
    combSum=zeros(1,t);
    [nodeSrc, nodeDst]
    for unionIdx = 2:n-1                                                   % for the 2nd term to the n-1 th term
        unionIdx
        combArr = nchoosek(1:n,unionIdx);                                   % union combinations
            for combIndex = 1:size(combArr,1)                               % cycle through the union combos
                ...
                for coverIdx = 1:t                                                           % for 1-100 hops
                    ...
                    combProbs = transMatrixT(nodeSrc,combArr(combIndex,:));                  % union probabilities
                    combSum(coverIdx) = combSum(coverIdx) +(-1)^(unionIdx-1)*sum(combProbs); % add to nth term sum sequence
                end
            end
    end
    ...
    %% Total Cover PDF sum
    coverTotal = cover_test + combSum +(-1)^(n-2)*ones(size(cover_test));
%     coverTotal = cover_test + combSum +(-1)^(n-1)*combEnd+1;
%     coverTotal = cover_test + combSum;
%     coverTotal = cover_test + combSum + combEnd;
    figure(3)
    hold on
    plot(0:100, [0 coverTotal],'--*')
    
%     coverTotal = [0 coverTotal];
%     
%     coverTotalPDF = zeros(1,t+1);
%     for idx = 1:t
%         coverTotalPDF(idx+1) = coverTotal(idx+1)-coverTotal(idx);
%     end
%     figure(2)
%     hold on
%     plot(0:100,[coverTotalPDF],'--*')
%     